...
import { getTeam, getDriver, RACES_2026, TEAM_HEADER_IMAGES } from "@/constants/f1Data";
...
export default function TeamDetailScreen() {
  ...
  const teamDrivers = team.drivers.map((dId) => getDriver(dId)).filter(Boolean);
  const headerImageUrl = TEAM_HEADER_IMAGES[team.id];
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.hero, { paddingTop: topPadding }]}>
        ...
        <LinearGradient
          colors={[team.color + "99", team.color + "44", "rgba(10,10,10,0.97)"]}
          style={StyleSheet.absoluteFillObject}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
        />
        <LinearGradient
          colors={["transparent", "transparent", "rgba(10,10,10,0.85)"]}
          style={StyleSheet.absoluteFillObject}
          start={{ x: 0, y: 0 }}
          end={{ x: 0, y: 1 }}
        />
