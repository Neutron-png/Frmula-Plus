      ...
      >
        ...
        {isLive && (
          <View style={styles.raceLiveBadge}>
            <Text style={styles.raceLiveBadgeText}>{t("explore.live")}</Text>
          </View>
        )}
        {status === "upcoming" && (
          <View style={styles.raceUpcomingBadge}>
            <Text style={styles.raceUpcomingText}>{t("explore.upcoming")}</Text>
          </View>
        )}
        <Ionicons
...
function SessionList({ meetingKey }: { meetingKey: number }) {
  ...
  if (!sessions || sessions.length === 0) {
    return (
      <View style={styles.sessionLoading}>
        <Text style={styles.sessionEmptyText}>{t("explore.sessions_unavailable")}</Text>
      </View>
    );
  }
...
export default function MoreScreen() {
  ...
  const tabLabels: Record<ExploreTab, string> = {
    calendar: t("explore.calendar"),
    circuits: t("explore.circuits"),
    teams: t("explore.teams"),
  };
    ...
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: topPadding + 8 }]}>
        ...
        <View style={styles.tabRow}>
          {(["calendar", "circuits", "teams"] as ExploreTab[]).map((tab) => (
            <TouchableOpacity
              key={tab}
            ...
            >
              {circuit.imageUrl && (
                <Image
                  ...
                  style={styles.circuitBgImage}
                  resizeMode="cover"
                />
              )}
              <LinearGradient
                colors={["rgba(15,15,15,0.85)", "rgba(15,15,15,0.65)"]}
                style={StyleSheet.absoluteFillObject}
              />
              <View style={styles.circuitLeft}>
                ...
                <View style={styles.circuitInfo}>
                  ...
                  <View style={styles.circuitStats}>
                    <Text style={styles.circuitStatItem}>{circuit.laps} {t("explore.laps")}</Text>
                    <Text style={styles.circuitStatDot}>·</Text>
                    <Text style={styles.circuitStatItem}>{circuit.length} {t("explore.km")}</Text>
                    <Text style={styles.circuitStatDot}>·</Text>
                    <Text style={styles.circuitStatItem}>{circuit.corners} {t("explore.corners")}</Text>
                  </View>
                </View>
              </View>
