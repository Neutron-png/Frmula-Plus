...
function DriverCard({ driver }: { driver: Driver }) {
  ...
  const headshotUri = getDriverPhoto(driver.id);
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: topPadding + 8 }]}>
        <Text style={styles.screenTitle}>{t("drivers.title")}</Text>
        <Text style={styles.screenSeason}>2026 · {allDrivers.length} {t("home.drivers")}</Text>
        ...
        <FlatList
          ...
          data={[{ id: null as string | null, shortName: t("drivers.all") }, ...TEAMS.map((team) => ({ id: team.id, shortName: team.shortName }))]}
          keyExtractor={(item) => item.id || "all"}
          contentContainerStyle={{ gap: 6, paddingBottom: 2 }}
          renderItem={({ item }) => {
            const team = item.id ? TEAMS.find((tm) => tm.id === item.id) : null;
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>{t("drivers.no_drivers_found")}</Text>
          </View>
        }
      />
    </View>
